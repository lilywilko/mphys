...
def main():
    ...
    X = 100   # run X simulations to collect data

    for k in range(50):
        # generates beta value based on R0 (beta * avg. neighbours = R0 = 1.5)
        R0=0.1*k
        beta=R0/np.mean(np.asarray(neighbour_nos))
        outbreaklengths=[]

        for j in range(X):
            print("Run no: " + str(j) + ", R0 = " + str(R0))
            # status arrays
        ...
        filename = 'R0_vs_outbreak_length.csv'
        file = open(filename,'a')
        if os.stat(filename).st_size == 0:
            file.write("Outbreak length,R0,Run no \n")
        for i in range(len(outbreaklengths)):
            file.write(str(outbreaklengths[i]) + "," + str(R0)+"," + str(i) +"\n")
        file.close()
